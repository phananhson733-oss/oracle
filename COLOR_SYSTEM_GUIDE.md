...
- **✅ 推荐使用 PNG/SVG 资源**：对于复杂图标，优先使用项目 `images/` 目录下的 PNG 或 SVG 资源。在小程序中，本地资源需注意包体积，大图或动态图建议通过 CDN 加载。
- **⚠️ Unicode 符号谨慎使用**：仅在极简场景下使用标准的 Unicode 占星/几何符号（如 ☉, ☽, ☿）。**必须**在真机测试其兼容性，部分低版本系统或特定安卓机型可能无法渲染。
- **✅ 视觉统一**：所有符号和图标必须指定项目定义的语义色（如 `text-accent` 或 `text-paper-600`）。

### 环境兼容性规范 (WeChat Mini Program)

- **非 Web 环境**：本项目为微信小程序，严禁直接使用 `window`, `document` 等 Web 特有 API（除非在特定兼容层中）。
- **组件限制**：UI 实现必须基于小程序 `<view>`, `<text>`, `<image>` 等基础组件，考虑 rpx 适配而非仅 px/rem。
- **资源路径**：代码中引用资源必须符合小程序路径规范（如 `/images/logo.png`），避免使用 Web 相对路径。

### 微信小程序布局安全区（最高优先级）

**右上角胶囊按钮是微信小程序的系统保留区域，严禁在此区域放置任何交互元素。**
...
| 规则 | 说明 |
|------|------|
| **❌ 禁止** | 在右上角放置按钮、图标、链接等可点击元素 |
| **❌ 禁止** | Header 使用三栏布局（左-中-右）将元素推到右上角 |
| **✅ 推荐** | 页面级操作按钮放在 Header 左侧或内容区域内 |
| **✅ 推荐** | 使用浮动按钮（FAB）放在页面右下角 |
| **✅ 推荐** | 将"档案"、"设置"等入口放在页面内容卡片中 |

**Header 布局规范**：
```html
<!-- ✅ 正确：简洁的 Header，操作按钮在左侧 -->
<view class="header">
  <view class="header-left">
    <view class="action-btn">档案</view>
  </view>
...
<view class="header">
  ...
  <view class="action-btn">档案</view>  <!-- 危险！ -->
</view>
```
