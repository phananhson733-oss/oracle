...
//        spy ChartMiniCalc / WikiChartCTA / SEO，控制 embeddedTool 开关。
// OUTPUT: tool-led 嵌入槽的行为契约：embeddedTool 在场 → 渲染 ChartMiniCalc（带正确
//         module/placement/fullChartHref 上游漏斗目标）并抑制底部 WikiChartCTA；
//         缺省文章 → 不渲染工具、保留底部 CTA。
...
vi.mock("../../components/wiki/WikiChartCTA", () => ({
  __esModule: true,
  default: () => <div data-testid="wiki-chart-cta" />,
}));
...
describe("WikiArticleDetailPage — tool-led 嵌入槽 (T4)", () => {
  it("embeddedTool 在场：渲染 ChartMiniCalc 并抑制底部 WikiChartCTA", async () => {
    ...
    expect(screen.queryByTestId("wiki-chart-cta")).toBeNull();
  });
  ...
  it("无 embeddedTool：不渲染工具，保留底部 WikiChartCTA", async () => {
    ...
    expect(await screen.findByTestId("wiki-chart-cta")).toBeTruthy();
