...
| `toolsCatalog.test.ts` | 守护 `components/tools/toolsCatalog.ts` 工具目录契约：恰 15 个公开工具、slug 唯一且命中真实路由 allowlist（无断链）、覆盖全部 routed slug（无遗漏工具）、每工具属唯一已声明分类、每分类非空、en/zh title+blurb 非空、blurb 无命运词（AI 安全红线 NO_FATE_CERTAINTY）。新增/移除公开工具或改路由 allowlist 须同步。 |
