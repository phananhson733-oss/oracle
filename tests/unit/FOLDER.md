...
| `tool-page-shell.test.tsx` | jsdom 组件测试：守护具体工具页使用宽版 `ToolPageShell`（默认 max-w-6xl），并在 SPA 水合后显示单工具自己的 summary / When to use / explainer / FAQ 内容，避免 SEO 正文只存在于静态 stub 而用户看不到；同时约束同一工具页的 guide/section/FAQ 可见标题不得重复。 |
