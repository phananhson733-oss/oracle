<!-- INPUT: 公共静态资源与图标目录（含 SEO 静态页、robots API/内部路径排除、hreflang 校验与站点图标）。 -->
<!-- OUTPUT: public 架构摘要与文件索引（含 SEO 静态页、robots API/内部路径排除、hreflang 校验与站点图标）。 -->
...
- brand｜地位：品牌标识派生图目录｜功能：导航/页脚小尺寸 logo 与结构化数据 512px logo，避免运行时请求原始大图。
