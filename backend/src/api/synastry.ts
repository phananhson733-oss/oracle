...
const resolveOverviewSection = (
  value: unknown,
): SynastryOverviewSection | null => {
...
const buildHouseCusps = (ascendantLon: number) =>
  Array.from({ length: 12 }, (_, i) =>
    normalizeLongitude(ascendantLon + i * 30),
  );
...
const buildHouseRulers = (positions: PlanetPosition[]) => {
  ...
  const startIndex = ascSign
    ? SIGN_ORDER.indexOf(ascSign as (typeof SIGN_ORDER)[number])
    : -1;
  const houseSigns =
    startIndex >= 0
      ? Array.from(
...
const buildExtendedNatalData = (
  ...
  ELEMENT_BODIES.forEach((planetName) => {
    ...
    elements[meta.element][meta.modality] =
      elements[meta.element][meta.modality] || [];
...
const calculateCrossAspects = (
  ...
  const byNameA = new Map(positionsA.map((p) => [p.name, p]));
  const byNameB = new Map(positionsB.map((p) => [p.name, p]));
  const listA = ASPECT_BODIES.map((name) => byNameA.get(name)).filter(
    Boolean,
  ) as PlanetPosition[];
  const listB = ASPECT_BODIES.map((name) => byNameB.get(name)).filter(
    Boolean,
  ) as PlanetPosition[];
...
const buildAspectLabel = (aspect: Aspect) =>
...
const pickTopOverlays = (
  ...
  const planetPriority = new Map(
    OVERVIEW_OVERLAY_PLANETS.map((planet, index) => [planet, index]),
  );
  const housePriority = new Map(
    OVERVIEW_OVERLAY_HOUSES.map((house, index) => [house, index]),
  );
  return overlays
...
const buildOverviewSynastrySignals = (
  aspects: Aspect[],
  overlays: SynastryData["houseOverlays"],
) => ({
  dimension_signals: {
    emotional_safety: pickAspectSignals(
      aspects,
      [
        ["Moon", "Moon"],
        ["Moon", "Venus"],
    ...
    communication: pickAspectSignals(
      aspects,
      [
        ["Mercury", "Mercury"],
        ["Mercury", "Sun"],
  ...
  sweet_signals: pickAspectSignalsByType(
    aspects,
    ["trine", "sextile", "conjunction"],
    OVERVIEW_SWEET_LIMIT,
  ),
  friction_signals: pickAspectSignalsByType(
    aspects,
    ["square", "opposition"],
    OVERVIEW_FRICTION_LIMIT,
  ),
  overlays_top: pickTopOverlays(
    overlays,
    OVERVIEW_OVERLAY_LIMIT,
    OVERVIEW_OVERLAY_HOUSES,
  ),
});

const buildHighlightsSignals = (
  aspects: Aspect[],
  overlays: SynastryData["houseOverlays"],
) => ({
  harmony_signals: pickAspectSignalsByType(
    aspects,
    ["trine", "sextile", "conjunction"],
    HIGHLIGHT_ASPECT_LIMIT,
  ),
  challenge_signals: pickAspectSignalsByType(
    aspects,
    ["square", "opposition"],
    HIGHLIGHT_ASPECT_LIMIT,
  ),
...
const buildHouseFocus = (positions: PlanetPosition[], houses: number[]) =>
  houses.reduce<Record<string, string[]>>((acc, house) => {
    acc[`house${house}`] = positions
      .filter((pos) => pos.house === house)
      .map((pos) => pos.name);
    return acc;
  }, {});
...
const summarizeCoreSynastryAspects = (aspects: Aspect[]) =>
  aspects
    .filter(
      (aspect) =>
        CORE_ASPECT_PLANETS.includes(aspect.planet1) ||
        CORE_ASPECT_PLANETS.includes(aspect.planet2),
    )
...
const scoreRelationshipTypes = (aspects: Aspect[]): SynastrySuggestion[] => {
  ...
  aspects.forEach((aspect) => {
    ...
    if (pairMatch(aspect.planet1, aspect.planet2, romanticPairs))
      scores.romantic += weight * 1.15;
    if (pairMatch(aspect.planet1, aspect.planet2, crushPairs))
      scores.crush += weight;
    if (pairMatch(aspect.planet1, aspect.planet2, friendPairs))
      scores.friend += weight;
    if (pairMatch(aspect.planet1, aspect.planet2, businessPairs))
      scores.business += weight * 0.95;
    if (pairMatch(aspect.planet1, aspect.planet2, familyPairs))
      scores.family += weight * 1.05;
  });
...
function parseBirthInput(
  ...
  return {
    ...
    lat:
      latParam === undefined || latParam === "" ? undefined : Number(latParam),
    lon:
      lonParam === undefined || lonParam === "" ? undefined : Number(lonParam),
...
const buildSynastryCore = async (birthA: BirthInput, birthB: BirthInput) => {
  ...
  const synastryAspects = calculateCrossAspects(
    chartA.positions,
    chartB.positions,
  );
...
const buildSynastrySummaryContext = (
  ...
  synastry: buildOverviewSynastrySignals(
    synastryAspects,
    synastry.houseOverlays,
  ),
...
const buildCoreDynamicsContext = (
  ...
  const compositePositions = buildCompositePositions(
    chartA.positions,
    chartB.positions,
  );
  return {
    ...
    synastry: {
      moon_venus_aspects: summarizeCoreSynastryAspects(synastryAspects),
    },
    comparison: {
...
synastryRouter.get("/technical", async (req, res) => {
  try {
    ...
    const { chartA, chartB, synastryAspects, overlaysAB, overlaysBA } =
    ...
    const technical: SynastryTechnicalData = {
      ...
      composite: buildExtendedNatalData(
        buildCompositePositions(chartA.positions, chartB.positions),
      ),
...
synastryRouter.post("/overview-section", authMiddleware, async (req, res) => {
  try {
    ...
    if (!section) {
      res.status(400).json({
        error:
          "Invalid section. Must be one of: core_dynamics, practice_tools, relationship_timing, highlights, vibe_tags, growth_task, conflict_loop, weather_forecast, action_plan.",
      });
      return;
    }
