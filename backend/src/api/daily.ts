...
dailyRouter.get("/", async (req, res) => {
  try {
    ...
    const technical = {
      transit_planets: transits.positions.filter((p) =>
        PLANETS.includes(p.name as any),
      ),
      transit_asteroids: transits.positions.filter((p) =>
        MINOR_BODIES.includes(p.name),
      ),
