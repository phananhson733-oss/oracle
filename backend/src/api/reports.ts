...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const hasAccess = await reportService.hasReportAccess(
        req.userId!,
        reportType,
      );
      const existingReport = await reportService.getReportByType(
        req.userId!,
        reportType,
      );
      ...
      const discount = entitlements.isSubscriber
        ...
        : 0;
      const price =
        discount > 0 ? Math.ceil(basePrice * (1 - discount)) : basePrice;
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!hasAccess) {
        return res
          .status(403)
          .json({ error: "Report not purchased", requiresPurchase: true });
      }
