// INPUT: Prompt 管理与版本策略（单语言 JSON、详情解读标签化分区与 CBT 行动建议约束；含 AI 安全护栏 withSafety 包装器）。
// OUTPUT: 导出 Prompt 加载与版本管理（snake_case 输出与版本化缓存，含详情解读分区标签、合盘成长焦点字段与全局 AI 安全护栏注入）。
...
export {
  ...
  resolveSafetyInstruction,
  resolveCbtDisclaimer,
  resolveNoFateReminder,
} from "./common.js";
...
/**
 * withSafety: 接收一个 PromptTemplate，返回新模板（不可变），并按 options 注入安全护栏。
 * - 始终在 system 字段最前面前置 SAFETY_INSTRUCTION（按 ctx.lang 选 zh/en）。
 * - 当 options.noFate === true 时，在 system 末尾追加 NO_FATE_CERTAINTY_REMINDER。
 * - 当 options.cbtFooter === true 时，在 user 输出末尾追加 CBT_DISCLAIMER_FOOTER 指令。
 *
 * 该 helper 是 51 个 prompt 的安全注入中枢——任何措辞调整都只需修改 common.ts 的常量，无需改动注册表。
 */
export function withSafety(
  template: PromptTemplate,
  options: { noFate?: boolean; cbtFooter?: boolean } = {},
): PromptTemplate {
  const originalSystem = template.system;
  const originalUser = template.user;

  const wrappedSystem = (ctx: Record<string, unknown>): string => {
    const safety = resolveSafetyInstruction(ctx);
    const base =
      typeof originalSystem === "function"
        ? originalSystem(ctx)
        : originalSystem;
    const fate = options.noFate ? `\n\n${resolveNoFateReminder(ctx)}` : "";
    return `${safety}\n\n${base}${fate}`;
  };

  const wrappedUser = options.cbtFooter
    ? (ctx: Record<string, unknown>): string => {
        const base = originalUser(ctx);
        const footer = resolveCbtDisclaimer(ctx);
        const footerInstruction =
          ctx.lang === "en"
            ? `\n\nIMPORTANT: At the very end of your output, append this exact disclaimer as a separate paragraph: "${footer}"`
            : `\n\n重要：请在你输出的最末尾追加以下声明作为独立段落（原文一字不差）："${footer}"`;
        return `${base}${footerInstruction}`;
      }
    : originalUser;
...
registerPrompt(
  withSafety({
    meta: { id: "natal-overview", version: "5.2", scenario: "natal" },
...
registerPrompt(
  withSafety({
    meta: { id: "natal-core-themes", version: "5.2", scenario: "natal" },
...
registerPrompt(
  withSafety({
    meta: { id: "natal-dimension", version: "5.2", scenario: "natal" },
...
registerPrompt(
  withSafety({
    meta: { id: "wiki-home", version: "1.1", scenario: "wiki" },
    system: (ctx) => {
      if (ctx.lang === "en") {
...
registerPrompt(
  withSafety({
    meta: { id: "wiki-classics-master", version: "1.1", scenario: "wiki" },
    system: (ctx) => {
      const lang = ctx.lang === "en" ? "en" : "zh";
      const domain = String(
        ctx.domain || (lang === "en" ? "psychological astrology" : "心理占星"),
      );
      ...
      const safeTitle =
        bookTitle || (lang === "en" ? "[Book Title]" : "[书名]");
...
const resolveAskCategory = (
  category?: string,
): keyof typeof ASK_CATEGORY_MODULES => {
...
registerPrompt(
  withSafety({
    meta: { id: "detail-elements-natal", version: "1.3", scenario: "natal" },
...
registerPrompt(
  withSafety({
    meta: { id: "detail-aspects-natal", version: "1.3", scenario: "natal" },
...
registerPrompt(
  withSafety({
    meta: { id: "detail-planets-natal", version: "1.3", scenario: "natal" },
...
registerPrompt(
  withSafety({
    meta: { id: "synthetica-analysis", version: "2.1", scenario: "wiki" },
    system: `
# Role
...
${SINGLE_LANGUAGE_INSTRUCTION}`,
    user: (ctx) => {
      ...
      const houseName =
        ctx.houseName || (lang === "en" ? "Not selected" : "未选择");
      const houseArchetype = ctx.houseArchetype || "";
      const topAspectsString =
        ctx.topAspectsString ||
        (lang === "en" ? "No major aspects" : "无主要相位");
      ...
      const contextTitle =
        lang === "en"
          ? "## 1. Context (Query Context)"
          : "## 1. Context (查询场景)";
      const focusLensLabel =
        lang === "en" ? "**Current Focus Lens:**" : "**当前聚焦透镜:**";
      const lensDefinitionLabel =
        lang === "en" ? "**Lens Depth Definition:**" : "**透镜深度定义:**";

      const inputDataTitle =
        lang === "en"
          ? "## 2. Input Data (Birth Chart Data)"
          : "## 2. Input Data (星盘数据)";
      const inputDataInstruction =
        lang === "en"
      ...
      const outputFormatTitle =
        lang === "en" ? "## 3. Output Format" : "## 3. Output Format";
...
6. **重要**：要点之间用"句号+空格"（。 ）分隔，不要使用换行符（\\n）。前端会自动解析并编号。`;
      ...
      const outputStructureLabel =
        lang === "en" ? "Output structure should include:" : "输出结构应包含：";
...
registerPrompt(
  withSafety({
    meta: { id: "detail-asteroids-natal", version: "1.3", scenario: "natal" },
...
registerPrompt(
  withSafety({
    meta: { id: "detail-rulers-natal", version: "1.3", scenario: "natal" },
