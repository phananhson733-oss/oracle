...
- `logger.ts`｜地位：结构化 logger｜功能：`createLogger({level})` 工厂 + 进程级默认 `logger`（级别读 `LOG_LEVEL` env，默认 info）。`error/warn/info/debug(msg, ctx?)` 输出一行 JSON `{level,msg,ts,...ctx}` 到对应 console sink；context 内部经 `sanitizeForLog` 兜底脱敏（隐私红线 #3 双保险），Error 展开为 `{name,message,stack}`。无新依赖，零冷启动崩溃。api/services 已全量从裸 console.* 迁移至此。
- `syntheticaConfig.ts`｜地位：Synthetica 配置归一｜功能：`normalizeSyntheticaConfig` 把 planet/sign/house/aspect 的 id 映射为 label + tier。

## 子目录

- `__tests__/`｜各工具的 vitest 单测（`sanitize-log.test.ts` / `syntheticaConfig.test.ts` / `logger.test.ts` 等）。
