...
class EmailService {
  ...
  async sendPaymentReceipt(email: string, details: {
    ...
    await this.send({
      to: email,
      subject: `AstroMind Payment Receipt — ${safe.amount} ${safe.currency}`,
  ...
  async sendPaymentFailedNotice(email: string, details: {
    ...
    await this.send({
      to: email,
      subject: 'AstroMind — Action Required: Payment Failed',
      html: this.buildBaseTemplate(
        'Payment Failed',
        ...
        <div style="background:#0f0f1a;border-radius:12px;padding:24px;margin:0 0 24px;">
          ...
          <p style="color:#a0a0b8;font-size:13px;margin:0;">Please update your payment method to continue enjoying AstroMind Pro features.</p>
        </div>
  ...
  async sendCancellationConfirmation(email: string, details: {
    ...
    await this.send({
      to: email,
      subject: 'AstroMind — Subscription Cancelled',
      html: this.buildBaseTemplate(
        'Subscription Cancelled',
        'Your subscription has been cancelled.',
        `
        <div style="background:#0f0f1a;border-radius:12px;padding:24px;margin:0 0 24px;">
          <p style="color:#a0a0b8;font-size:13px;margin:0 0 8px;">Your AstroMind Pro subscription has been cancelled.</p>
    ...
    <tr><td align="center">
      <table width="480" cellpadding="0" cellspacing="0" style="max-width:480px;width:100%;background:#1a1a2e;border-radius:16px;border:1px solid rgba(212,175,55,0.2);">
        <tr><td style="padding:40px 32px;text-align:center;">
          <h1 style="color:#d4af37;font-size:24px;margin:0 0 4px;">AstroMind</h1>
      ...
      <p style="color:#666680;font-size:11px;margin:16px 0 0;text-align:center;">&copy; ${new Date().getFullYear()} AstroMind. All rights reserved.</p>
    </td></tr>
  </table>
</body>
</html>`;
