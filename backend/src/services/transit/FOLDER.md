...
- lifeArc.ts｜地位：人生 K 线（年级）引擎（异步 + 纯核心）｜功能：detectReturnMarkers（周期播种 Saturn/Jupiter/Nodal 返照 + Uranus 对冲，纯）、assembleLifeCandles（季度采样→归一化→年级蜡烛，纯）、buildLifeTimeline（慢速外行星集 + 固定参考跨度基线缓存 + **结果级缓存**（键 `transit:lifearc:result:{sha256(计算投影 date/time/accuracy/lat/lon/timezone——不含 city 自由文本)}:{from}-{to}:{TIMELINE_ALGO_VERSION}`，30d TTL，命中免重算 ~400 次年度采样；命中侧 isReusableLifeResult 校验根数/蜡烛最小契约/markers；hasPartial 或空 candles 不落缓存，B10）+ 进程内 single-flight（同冷键并发合并为一次计算）+ 完整性门；缓存读写失败经 logger.warn 可观测）。
- lifeArc.test.ts｜地位：人生 K 线核心算法单测（9 测：标记年龄、年级蜡烛装配、基线归一化、区间摘要、topAspects、phase unknown）。
- lifeArc.contract.test.ts｜地位：前端 v7 呈现层契约 pin + 结果级缓存契约（22 测，test-only）｜功能：钉住 buildLifeTimeline(0,99) 恰 100 根/age 连续/字段有限 0-100、domainScores 恒 undefined、detectReturnMarkers(99) SR≈29/59/88 + Uranus opp≈42、MAX_LIFE_CANDLES===100、contract.semantics==="interval-summary"、topAspects 含 natalBody 且 natalBody/transitBody 均属规范天体名集合（Sun…Pluto + "North Node"，前端 MODULE_AFFINITY 精确串匹配的钉子）、result.markers 本身非空/全在 fromAge..toAge 内/含 SR 三连——`components/timeline/lifekline/` 的全部数据假设；另钉结果级缓存 7 契约（冷路径恰一次 set + 30d TTL、键含 algo version + sha256 无出生数据明文、同参二次调用命中缓存零星历重算、partial/mock fallback 不落缓存、city 变体文本不换键、畸形条目视同 miss 重算、同冷键并发 single-flight 合并）；后端改动破坏任一假设即红。
...
近期更新
- 2026-07-17 /ship 对抗评审加固（Claude+Codex 双模型）：① 结果缓存键改哈希计算投影（date/time/accuracy/lat/lon/timezone）——city 是 ≤200 字符自由文本且坐标解析后不参与星历计算，进键可被变体文本无限铸新键灌缓存；② 命中侧 isReusableLifeResult 校验（根数=区间长度、每根 age/intensity 有限 + topAspects 数组、markers 数组），污染条目视同 miss；③ 进程内 single-flight Map 合并同冷键并发（一次冷算 ≈760 次同步采样）；④ 写侧增 candles.length>0 护栏 + timeline.ts route 层 fromAge 同步 clamp 0-99（纯未来区间原可制造 fromAge>toAge 空结果污染缓存）；⑤ 缓存读写 catch 改 logger.warn 可观测。contract 测试 19→22（city 不变性/畸形条目重算/single-flight）。
- 2026-07-17 lifeArc.ts 结果级缓存：buildLifeTimeline 开头查 `transit:lifearc:result:{sha256(birth)}:{fromAge}-{toAge}:{TIMELINE_ALGO_VERSION}`，命中直接返回（收敛匿名 0-99 岁请求每次 ~400 次年度采样的放大面）；计算后 30d TTL best-effort 写入；hasPartial（任一年份走 mock fallback）不写缓存（B10 不缓存假数据；判 hasPartial 而非 dataQuality==="partial"，因时间未知时会被 approximate_time 覆盖）。TDD：contract 测试 12→16（RED→GREEN），tsc 干净。
