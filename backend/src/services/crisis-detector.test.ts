...
describe("detectCrisis - edge cases", () => {
  ...
  it("returns no-hit on array of undefined/null/empty", () => {
    expect(
      detectCrisis([undefined, null, "", "   "] as unknown as string[]).hit,
    ).toBe(false);
  });
  ...
  it("never throws even on weird input", () => {
    expect(() => detectCrisis(null as unknown as string[])).not.toThrow();
  });

  it("fails CLOSED on null input (security: detector error treated as crisis)", () => {
    // Intentional security upgrade: detector exceptions return {hit:true, failSafe:true}
    // so callers short-circuit to the safe response (helpline) instead of forwarding
    // user text to the LLM. See CLAUDE.md §AI 安全边界 #3.
    const result = detectCrisis(null as unknown as string[]);
    expect(result.hit).toBe(true);
    expect(result.failSafe).toBe(true);
    expect(result.reason).toBe("detector-error");
  });

  it("returns no-hit on empty (well-formed) input", () => {
    const result = detectCrisis([]);
    expect(result.hit).toBe(false);
    expect(result.failSafe).toBeUndefined();
  });
});
