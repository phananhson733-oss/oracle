<!-- INPUT: 后端服务目录结构与职责说明（snake_case 输出，含经典拆解 Markdown 数据源、报告积分购买、地理搜索优化与手动 Pro 试用迁移）。 -->
<!-- OUTPUT: 后端服务目录文档（含经典拆解数据源、报告积分购买、地理搜索优化与手动 Pro 试用迁移说明）。 -->
...
## 近期更新
- 新增手动 Pro 试用迁移：注册 RPC 不再自动写入 `trial_ends_at`，新增 `pro_trial_claims` 用于 Airwallex-backed trial 防重复。
