// Article: How North Node vs South Node Maps Your Growth Edge
...
The nodes are calculated points, so an accurate birth time and a chart calculator give you both ends at once. Many readers begin with [guide to finding the North Node sign and house](/en/wiki/astrology-houses) before going deeper.
...
- [comparison of the North Node across all twelve signs](/en/wiki/north-node-vs-south-node) — see how the growth direction shifts sign by sign
- [guide to the South Node and releasing past patterns](/en/wiki/north-node-vs-south-node) — go deeper on working consciously with the past pole
...
北交点和南交点（North Node 与 South Node）是星盘上**一对标记成长方向与舒适区的相对点**。它们不是行星，而是月亮轨道与黄道相交算出来的两个点，所以又叫月亮交点，始终相隔一百八十度，构成一条慢慢平衡的轴。读懂这对交点的关键，是把它放回整张盘里看，可以先翻一翻 [出生星盘入门总览 pillar 页](/zh/wiki/how-to-read-birth-chart)，再回头读这条轴。
...
把这对交点当成一组自查的入口，比急着给自己贴「我天生内向／外向」的标签有用得多。它提醒的从来不是你是谁，而是此刻你更该练哪条肌肉，这也正是 [交点轴与人生成长方向解读](/zh/wiki/north-node-vs-south-node) 想讲清楚的事。
...
- [南交点解读与运用指南](/zh/wiki/north-node-vs-south-node) — 想单独把南交点这一端讲透，看清「舒适区」该怎么用而不是怎么丢，可以从这篇入手。
- [北交点各星座落点对照](/zh/wiki/north-node-vs-south-node) — 各星座落点的差别都不一样，这篇按星座一条条拆开对照，方便你对上自己的盘。
- [行运交点与人生节点对照](/zh/wiki/north-node-vs-south-node) — 交点每隔一段会走到星盘的关键位置，这篇梳理它带来的成长节点，和本命交点是两回事。
