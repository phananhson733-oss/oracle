// Article: July 2026 Planetary Transits: Jupiter Enters Leo While Mercury Retrograde Finally Lifts
// Generated from flow-mvp _staging/ by tools/scripts/gg-md-to-oracle-ts.mjs.
import type { WikiArticle } from "../../types";

export const july2026PlanetaryTransitsEn: WikiArticle = {
  slug: "july-2026-planetary-transits",
  image: "/images/transits/july-2026-planetary-transits.jpg",
  image_alt:
    "A large warm-gold planet rising as the focal point of a deep indigo sky, with smaller dim bodies quietly turning at the edges.",
  title:
    "July 2026 Planetary Transits: Jupiter Enters Leo While Mercury Retrograde Finally Lifts",
  description:
    "Every dated event for July 2026, drawn straight from the ephemeris. Times are in UT, so adjust for your location.",
  authorId: "marcus-orion",
  date: "2026-06-01",
  schema: "Article",
  lang: "en",
  keywords: [],
  content: `# July 2026 Planetary Transits: Jupiter Enters Leo While Mercury Retrograde Finally Lifts

July 2026 opens with the kind of marker the sky only hands out about once a year. On July 1, Jupiter leaves the sign it has been crossing and moves into Leo. That is a once-a-year ingress, and it resets the backdrop for the months ahead. It is the headline event of the month, and everything else plays out underneath it.

The month is also bracketed by a Mercury retrograde that does not belong to July alone. Mercury turned retrograde back on June 30 and stays that way until July 24, when it stations direct at 16 degrees of Cancer. The first three weeks carry the familiar drag of a retrograde, and the relief arrives only in the final stretch. Reading the month well means not mistaking that early friction for a verdict about July as a whole.

Then, in the back half, two slow outer planets quietly change direction. Neither is dramatic the way an ingress is, but together they mark a turn in the month's center of gravity. None of this is a forecast of what will happen to you. It is a description of the sky's moving parts, offered as observations you can check against your own week.

## Key Dates at a Glance

Every dated event for July 2026, drawn straight from the ephemeris. Times are in UT, so adjust for your location.

| Date | Event | Plain-language read |
| --- | --- | --- |
| July 1 | Jupiter enters Leo | A once-a-year ingress; Jupiter begins its roughly year-long pass through Leo. |
| July 8 | Neptune stations retrograde at 4° Aries | The first of the month's two outer-planet turns. |
| July 10 | Venus enters Virgo | A shift in how Venus expresses its themes for the next few weeks. |
| July 15 | New Moon in Cancer | The month's first lunation, a fresh cycle in the sign of home and roots. |
| July 23 | Sun enters Leo | The Sun joins Jupiter in Leo as the solar season turns. |
| July 24 | Mercury stations direct at 16° Cancer | The retrograde that began June 30 finally lifts. |
| July 27 | Saturn stations retrograde at 15° Aries | The second outer-planet turn of the month. |
| July 30 | Full Moon in Aquarius | The month's closing lunation, opposite the Leo season just begun. |

To see which house each date falls into for you, lay this table beside your own chart and read it alongside the overview of [astrological transits and how they move through your chart](/en/wiki/transits). The same date lands very differently depending on where it touches your placements. For the full year in one place, see the [2026 astrology calendar](/en/wiki/2026-astrology-calendar).


![A timeline of July 2026's eight key transit dates from Jupiter entering Leo to the July 30 Aquarius full moon.](/images/transits/july-2026-planetary-transits-i0-en.svg)
## Jupiter Enters Leo: A Once-a-Year New Chapter

The July 1 Jupiter ingress is the event worth slowing down for, because Jupiter only changes signs about once a year. By mid-month the background chart shows Jupiter still early in the sign, at 3 degrees of Leo, so this is the opening of a long passage, not its peak. Here is how to work with it:

1. **Find the house.** Locate which house Leo occupies in your own chart — that house is the life area getting Jupiter's expansive attention for roughly the next year.
2. **Watch for expansion.** Jupiter tends to enlarge whatever it touches, for better and worse. In that house, themes can grow, opportunities can open, and confidence can run ahead of the facts.
3. **Pressure-test the optimism.** Notice where you suddenly feel more willing to take up space, and ask whether that expansion is grounded in something real or just the optimism of a fresh transit.

Jupiter does not promise luck. It describes a season where one domain of your life gets louder and more generous, and your job is to put that to use deliberately rather than wait for it to happen to you.

## Mercury Retrograde Closes July 24, and a Cancer New Moon Resets

For the first three weeks of July, Mercury is still retrograde. The cycle began June 30 and does not end until it stations direct on July 24 at 16 degrees of Cancer. At mid-month the chart puts Mercury at 20 degrees of Cancer, retrograde, so the slowdown sits squarely in the sign of home, memory, and family.

The observable pattern is well worn, and so is the practical response:

1. **Conversations circle back.** Give important conversations a second read rather than treating the first pass as final.
2. **Agreements need re-checking.** Expect to revisit and refine rather than finalize.
3. **Hasty decisions reopen.** Treat these weeks as time for tightening what exists, not launching what does not.

Sitting inside that window is the New Moon in Cancer on July 15. A new moon starts a lunar cycle, and in Cancer it points toward home, roots, and the people you treat as family. Because it lands while Mercury is still retrograde, the cleaner read is not "launch something brand new" but "quietly begin reworking something close to home."

By July 24, when Mercury stations direct, the friction lifts and the loose threads from those three weeks become easier to tie off. To understand how a passing transit like this interacts with your own birth placements, the explainer on [natal chart transits and how to read them against your birth chart](/en/wiki/natal-chart-transits) is the place to start.

Two July peaks are worth marking on the chart. The [Aquarius full moon at the end of July](/en/wiki/full-moon-july-2026) arrives just after Mercury turns direct, making it a natural point to integrate whatever the retrograde stirred up, and the [2026 World Cup reaches its final under the same summer sky](/en/wiki/world-cup-2026-astrology-prediction) — a useful lens on the collective mood carrying the month.

## Neptune and Saturn Both Station Retrograde: The Outer Planets Turn Inward

The back half of July carries two slow turns that are easy to miss because nothing about them is loud:

1. **Neptune stations retrograde (July 8).** It turns at 4 degrees of Aries; at mid-month it sits there, barely moving.
2. **Saturn stations retrograde (July 27).** It turns at 15 degrees of Aries, already parked at that degree by mid-month.

A station is the moment a planet appears to stop before changing direction, and it concentrates that planet's themes. With both in early Aries, their stations are approximate near-neighbors in the sky — not an exact aspect with a precise date, but close enough in degree to be worth naming together. Treat it as a rough proximity and a backdrop, not a precise event.

The observable shift is one of direction. Retrograde periods for the slow planets are traditionally read as a turn from outward action toward internal review — less building in the world, more re-examining what you have already built. For Saturn especially, that means revisiting the structures and limits you set earlier; the retrograde is where you pressure-test whether they hold.

For a fuller account of how Saturn's longer cycle works, see the deep dive on *Saturn transits and the structure they ask you to build*. The takeaway across both stations is the same: late July rewards reviewing over launching.

## Sun Enters Leo and an Aquarius Full Moon: The Two-Sign Axis

On July 23 the Sun enters Leo, joining Jupiter there and turning the solar season toward the sign of visibility and self-expression. A week later, on July 30, the Full Moon in Aquarius closes the month. A full moon always sits opposite the Sun, so this one lights up the Leo-Aquarius axis end to end:

- **Leo** is the individual stepping forward — visibility, self-expression, the part of you that wants to be seen.
- **Aquarius** is the group, the wider community, the part of you that belongs to something larger.

A full moon brings a theme to a head, and on this axis the theme is the tension between standing out and fitting in. Notice where that pull is live right now: a project where personal recognition and group contribution compete, or a gap between what you want for yourself and what your community expects.

The full moon does not resolve that tension; it makes it visible enough to name. Use it as a checkpoint, not a verdict — a moment to see which side of the axis has been carrying more weight, and whether that balance is the one you actually want.

## How to Work With July's Transits

The most useful thing you can do with this month is also the most concrete: find out which house each of the eight dates falls into in your own chart. Every transit is a moving planet crossing a fixed map — your birth chart — and the meaning is decided almost entirely by where it lands.

Here is a method you can run:

1. **Pull up your birth chart.** Have it open beside the Key Dates table above.
2. **Locate three signs.** Find which house holds Leo (where Jupiter's ingress and the late-month Sun are working), which holds Cancer (where Mercury's retrograde and the July 15 New Moon concentrate), and which holds early Aries (where Neptune and Saturn are stationing).
3. **Write the three house numbers down.** These are the two or three places the month actually touches your life.
4. **Read each date against its house, not as a headline.** A Cancer New Moon means little in the abstract; landing in your fourth house versus your tenth changes the question it asks.

This narrows a month of sky-wide events down to the few places they actually touch your life. Watch those, and let the rest pass.

The discipline worth keeping is to treat all of this as a language for observing your own experience, not a forecast of what is coming. A transit is useful when it helps you name something you had already half-noticed and check it against a real week. It stops being honest the moment it becomes a prediction you stop questioning.


## Questions People Ask About July 2026 Planetary Transits

**What happens when Jupiter enters Leo on July 1, 2026?**

On July 1, Jupiter leaves the sign it has been crossing and moves into Leo, a once-a-year ingress that resets the backdrop for the months ahead. By mid-month it sits early in the sign, at 3 degrees of Leo, so this is the opening of a roughly year-long passage rather than its peak.

**When does Mercury retrograde end in July 2026?**

Mercury turned retrograde back on June 30 and stays that way until July 24, when it stations direct at 16 degrees of Cancer. The first three weeks carry the familiar drag of a retrograde, and the relief arrives only in that final stretch.

**What does the New Moon in Cancer on July 15 mean?**

The New Moon in Cancer on July 15 starts a lunar cycle pointing toward home, roots, and the people you treat as family. Because it lands while Mercury is still retrograde, the cleaner read is not to launch something brand new but to quietly begin reworking something close to home.

**Which signs does the Full Moon on July 30 affect?**

On July 30 the Full Moon in Aquarius closes the month, sitting opposite the Sun that entered Leo on July 23, so it lights up the Leo-Aquarius axis end to end. The theme on this axis is the tension between standing out and fitting in, offered as a checkpoint rather than a verdict.

## Take Action

The fastest way to make July 2026 useful is to stop reading these transits in the abstract and locate them on your own chart. If you have never mapped the houses against the planets, start with the basics. [Read the full guide to reading a birth chart to see where July's transits land for you.](/en/wiki/how-to-read-birth-chart)

For the structural map of which life area each date touches, the overview of [the astrological houses and what each one governs](/en/wiki/astrology-houses) is the companion piece to keep open beside this one.
`,
};

export const july2026PlanetaryTransitsZh: WikiArticle = {
  slug: "july-2026-planetary-transits",
  image: "/images/transits/july-2026-planetary-transits.jpg",
  image_alt:
    "一颗暖金色的大行星在深靛蓝星空中升起成为焦点，边缘几颗暗淡的星体悄悄转向。",
  title: "2026 年 7 月行运预报：木星入狮子，上旬水逆终于在月末收尾",
  description:
    "下面是 2026 年 7 月每一个有确切日期的事件，直接取自星历。时间为 UT（世界时），请按你所在地时区换算。",
  authorId: "marcus-orion",
  date: "2026-06-01",
  schema: "Article",
  lang: "zh",
  keywords: [],
  content: `# 2026 年 7 月行运预报：木星入狮子，上旬水逆终于在月末收尾

2026 年 7 月开篇就给出一个天空大约一年才发一次的标记。7 月 1 日，木星离开它一直行经的星座，进入狮子座。这是一年一度的换座，会重置接下来数月的背景盘。它是本月的头条事件，其余一切都在它之下展开。如果你一直觉得下半年像是想翻开新的一章，那么日历指向的就是这一天。

本月还被一段并不只属于 7 月的水星逆行夹在中间。水星早在 6 月 30 日就开始逆行，一直持续到 7 月 24 日，才在巨蟹座 16 度顺行。本月前三周带着熟悉的逆行拖滞感，而松绑只在最后一段才到来。把这个月读对，意味着不要把上旬的这股摩擦当成对整个 7 月的判决。

接着，在月份后半段，两颗移动缓慢的外行星悄悄转向。两者都不像换座那样是戏剧化、看得见的事件，但合在一起，它们标记了本月重心的一次转折。这些都不是对你将发生什么的预言。它是对天空运转部件的一种描述，作为一组你可以拿自己这一周来核对的观察。

## 关键日期一览

下面是 2026 年 7 月每一个有确切日期的事件，直接取自星历。时间为 UT（世界时），请按你所在地时区换算。

| 日期 | 事件 | 白话解读 |
| --- | --- | --- |
| 7 月 1 日 | 木星入狮子座 | 一年一度的换座；木星开始为期约一年的狮子座行程。 |
| 7 月 8 日 | 海王星起逆行，白羊座 4 度 | 本月两次外行星转向中的第一次。 |
| 7 月 10 日 | 金星入处女座 | 接下来几周金星表达其主题方式的一次转变。 |
| 7 月 15 日 | 巨蟹座新月 | 本月第一个月相，在家庭与根脉之座开启的新周期。 |
| 7 月 23 日 | 太阳入狮子座 | 随着太阳季更替，太阳加入位于狮子座的木星。 |
| 7 月 24 日 | 水星顺行，巨蟹座 16 度 | 6 月 30 日开始的水逆终于结束。 |
| 7 月 27 日 | 土星起逆行，白羊座 15 度 | 本月第二次外行星转向。 |
| 7 月 30 日 | 水瓶座满月 | 本月收尾的月相，与刚开启的狮子座季节相对。 |

要看清这些日期对你个人分别落在哪一宫，把这张表摆在你自己的星盘旁，配合 [占星行运 astrological transits 如何穿行你的星盘](/zh/wiki/transits) 这篇总览一起读。同一个日期，落在不同位置，呈现会非常不同。


![2026 年 7 月八个关键行运日期的时间线，从木星入狮子到 7 月 30 日水瓶满月。](/images/transits/july-2026-planetary-transits-i0-zh.svg)
## 木星入狮子：一年一度的新篇章

7 月 1 日的木星换座，是值得放慢脚步看的事件，因为木星大约一年才换一次星座。到月中，背景盘显示木星仍在该星座早段，位于狮子座 3 度，所以这是一段长行程的开端，而非其顶点。具体可以这样用：

1. **找出宫位。** 找出狮子座在你自己星盘里占据哪一宫——那一宫就是接下来约一年里，得到木星扩张式、放大式关注的生活领域。
2. **留意扩张。** 木星往往放大它所触及的一切，好坏皆然。在那一宫，主题可能扩张，机会可能打开，自信也可能跑在事实前面。
3. **给乐观做压力测试。** 留意你在哪里突然觉得更想伸展、更愿意占据空间，并问自己这种扩张是扎根于某个真实的东西，还是仅仅是一段新行运带来的乐观。

木星不承诺好运。它描述的是某段时期里你生活的某个领域变得更响亮、更慷慨，而你的任务是有意识地加以利用，而不是等它降临到你头上。

## 水逆 7 月 24 日收尾，巨蟹座新月重置开局

7 月的前三周，水星仍在逆行。这个周期从 6 月 30 日开始，直到 7 月 24 日在巨蟹座 16 度顺行才结束。月中星盘把水星放在巨蟹座 20 度、逆行，所以这段拖滞稳稳落在家庭、记忆与亲情之座。

水逆可观察到的模式人尽皆知，对应的做法也很直白：

1. **对话兜回原点。** 给重要对话留出再读一遍的余地，别把第一遍当定稿。
2. **约定需要再核一遍。** 预期是返工与打磨，而非一次定稿。
3. **仓促的决定被重新打开。** 把这几周当作收紧已有之物、而非启动尚无之物的时期。

落在这个窗口之内的，是 7 月 15 日的巨蟹座新月。新月是一个月相周期的开始，而在巨蟹座，它指向家、根脉，以及你视为家人的那些人。因为它落在水星仍逆行之时，更干净的读法不是「启动全新的东西」，而是「悄悄开始重做某件贴近家的事」。

到了 7 月 24 日水星顺行，摩擦松绑，那三周里散落的线头会更容易收尾。如果你想理解这样一段过路行运如何与你自己的出生位置互动，关于 [本命盘行运 natal chart transits 如何对照出生盘来读](/zh/wiki/natal-chart-transits) 的讲解是入手的起点。

## 海王星与土星相继起逆：外行星转向内省

7 月后半段带着两次缓慢的转向，容易被错过，因为它们一点都不张扬：

1. **海王星起逆行（7 月 8 日）。** 它在白羊座 4 度转向；月中它位于白羊座 4 度、逆行，几乎不动。
2. **土星起逆行（7 月 27 日）。** 它在白羊座 15 度转向；月中星盘显示它已停驻在同一星座的 15 度。

「逆留」是一颗行星在改变方向前看似停下的那一刻，它往往会浓缩这颗行星的主题。由于本月这两颗行星都占据白羊座早段，它们的逆留在天空中是近似的相邻位置——这不是带精确日期的精确相位，但在度数上足够接近，值得放在一起来提。诚实的说法是，这是一种大致的接近，而非干净利落的事件，所以把它当作背景、而非头条来对待。

可观察到的变化是方向上的。对缓慢行星而言，逆行期传统上被读作从向外行动转向向内复盘——少在世界里搭建，多重新审视你已经搭好的东西。尤其对土星来说，这意味着重访你早先设下的结构、承诺与界限；逆行正是你压力测试它们是否真的站得住的时候。

要更完整地了解土星这一较长周期如何运作，可参看关于 *土星行运 Saturn transits 与它要你搭建的结构* 的深入解读。两次逆留通用的应用是同一句话：7 月下旬奖励复盘，不奖励启动。

## 太阳入狮子与水瓶座满月：两极之轴

7 月 23 日，太阳进入狮子座，加入已在那里的木星，把太阳季转向能见度、自我表达与被看见之座。一周之后，7 月 30 日的水瓶座满月为本月收尾。满月总是与太阳相对，这意味着这次满月把狮子—水瓶这条轴线从头到尾点亮：

- **狮子座** 是上前一步的个体——能见度、自我表达、你身上想被看见的那一面。
- **水瓶座** 是群体、是更广的社群、是你身上属于某个更大整体的那一部分。

满月往往把一个主题推到顶点，而在这条轴线上，这个主题是突出与融入之间的张力。留意这股拉力此刻在你身上哪里活跃：某个个人认可与团队贡献相互竞争的项目，或某段关系里你想为自己要什么、与你的社群期待什么之间的拉扯。

满月并不化解这股张力；它只是让它清晰到足以被命名。把它当作一个检查点，而非判决——一个看清近来狮子—水瓶轴线哪一端承载了更多分量、而那个平衡是不是你真正想要的时刻。

## 如何运用 7 月的这些行运

这个月你能做的最有用的一件事，也是最具体的一件：把上面那八个日期拿出来，逐一查出它们在你自己星盘里各落在哪一宫。本指南里的每一项行运，都是一颗移动的行星穿过一张固定的地图——你的出生盘——而含义几乎完全由它落在哪里来决定。

下面是一个你真的能跑起来的方法：

1. **打开你的出生盘。** 把它摆在上面那张关键日期表旁边。
2. **定位三个星座。** 哪一宫装着狮子座（木星换座与月末太阳在运作之处），哪一宫装着巨蟹座（水星逆行与 7 月 15 日新月集中之处），以及哪一宫装着白羊座早段（海王星与土星起逆之处）。
3. **把这三个宫位号写下来。** 这就是本月真正与你生活相交的那两三个地方。
4. **把每个日期对照它所触及的那一宫来读，而不是当作悬空的头条。** 巨蟹座新月在抽象层面意义不大；落在你的第四宫还是第十宫，会改变它所提出的整个问题。

这把一个布满全天空事件的月份，收窄到它们真正与你生活相交的那几个地方。盯住那些，让其余的过去。

值得守住的纪律是：把这一切当作观察自己经验的一套语言，而不是对将至之事的预报。一项行运之所以有用，是当它帮你说出某件你早已隐约察觉的事，并拿真实的一周去核对它。它在变成一个你不再质疑的预言的那一刻，就不再诚实。


## 关于 2026 年 7 月行运的常见问题

**2026 年 7 月 1 日木星入狮子座会发生什么？**

7 月 1 日，木星离开它一直行经的星座，进入狮子座，这是一年一度的换座，会重置接下来数月的背景盘。到月中它仍在该星座早段，位于狮子座 3 度，所以这是一段约一年长行程的开端，而非其顶点。

**2026 年 7 月水星逆行什么时候结束？**

水星早在 6 月 30 日就开始逆行，一直持续到 7 月 24 日，才在巨蟹座 16 度顺行。本月前三周带着熟悉的逆行拖滞感，而松绑只在最后一段才到来。

**7 月 15 日的巨蟹座新月意味着什么？**

7 月 15 日的巨蟹座新月开启一个月相周期，指向家、根脉，以及你视为家人的那些人。因为它落在水星仍逆行之时，更干净的读法不是「启动全新的东西」，而是「悄悄开始重做某件贴近家的事」。

**7 月 30 日的满月影响哪些星座？**

7 月 30 日的水瓶座满月为本月收尾，与 7 月 23 日进入狮子座的太阳相对，因此把狮子—水瓶这条轴线从头到尾点亮。这条轴线上的主题是突出与融入之间的张力，它是一个检查点，而非判决。

## 立即行动

让 2026 年 7 月真正有用的最快办法，是别再抽象地读这些行运，而是把它们定位到你自己的星盘上。如果你从没把宫位与行星对照过，就从基础开始。[阅读完整的出生盘解读指南，看清 7 月的行运具体落在你哪里。](/zh/wiki/how-to-read-birth-chart)

要拿到那张告诉你每个日期触及哪个生活领域的结构地图，关于 [占星宫位 astrology houses 各宫分别主管什么](/zh/wiki/astrology-houses) 的总览，是值得开着、摆在这篇旁边的同伴篇。
`,
};
