// Article: How the Core Astrology Terms Fit Together in One Chart
...
1. **Find the four angles first.** Locate the Ascendant, Midheaven, Descendant, and IC; the [explainer on the descendant angle](/en/wiki/descendant-astrology) marks your one-to-one relationships and the [explainer on the IC angle, the Imum Coeli](/en/wiki/ic-astrology) marks your private roots and home base.
...
**Why do these labels seem to contradict each other?**

Because each one is a single factor in a whole chart, and factors modify each other. A "lazy" trine can sharpen when it touches a demanding angle, so the contradiction usually means two factors are interacting rather than competing.
...
- [overview of the major and minor chart aspects](/en/wiki/astrology-terms) — for the full set of shapes beyond the three covered here
- [guide to the twelve astrological houses](/en/wiki/astrology-houses) — for where each word lands across the wheel
...
破解的办法只有一个：把每个词都读成一段关系的一半，而不是一条固定的性格标签。沿着 Dane Rudhyar 开创的那一脉以人为本的读法，三分相不是一个孤立的「好形状」，它是两颗具体行星、落在两个具体宫位里的连线，配置一变，含义就变。想看这套词汇怎么在一整张盘里同时运转，可以放进更上位的 [本命星盘阅读入门总览](/zh/wiki/how-to-read-birth-chart) 里一起读。

## 如何在自己身上识别 Astrology Terms

要在自己的盘里认出 Astrology Terms 并真正用起来，最好按顺序跑一份短清单，而不是堆一摞定义。打开你的星盘，逐条找下面这些信号：

1. **先找四个轴点。** 定位上升、天顶、下降、天底；其中 [下降点轴点的含义解析](/zh/wiki/descendant-astrology) 标记你的一对一关系，而 [天底（Imum Coeli）轴点解析](/zh/wiki/ic-astrology) 标记你的根基与归属感。
...
- [星盘主要与次要相位总览](/zh/wiki/astrology-terms) —— 想看本文三个之外的完整相位清单，从这里入手
- [占星十二宫位完整指南](/zh/wiki/astrology-houses) —— 想弄清每个词最终落在盘的哪一块，对照宫位读更顺
