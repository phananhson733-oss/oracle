<!-- INPUT: /landing-v2 页面分段组件清单（Hero 立即加载 + HeroTodayCard idle 后加载 + 首屏外 React.lazy 分段 + viewport-deferred loading）。 -->
<!-- OUTPUT: pages/landing 子目录架构摘要与文件索引（含 PageSpeed 首屏与首屏外分段延迟加载记录）。 -->
...
- LandingPage.tsx 仅做组合 + SEO 元；Hero 立即加载，Hero 右半区编辑卡 idle 后加载，其余首屏外分段 React.lazy + Suspense + IntersectionObserver viewport-deferred + 占位高度防 CLS。
- 数据共享：idle 后加载的 Hero 右半区编辑卡 (HeroTodayCard) 与 CosmicWeatherSection 经由 hooks/useTodaySky 共用一次 /api/astro/today 请求。
...
- HeroSection.tsx｜地位：首屏区块｜功能：Editorial Serif Poster 标题（LCP 使用系统衬线避免等 Web Font）+ 双 CTA（统一收敛到 BirthChart anchor）+ md+ 7/5 双列网格右半区 idle 后嵌入 HeroTodayCard + 5 个 keyword feature-pills（Free Birth Chart / Today's Sky / Synastry / Saturn Return / Ask Oracle，4 个站内锚点 + 1 个 Saturn Return 路由到独立 SEO 页）。
...
近期更新
- PageSpeed 优化：LandingPage 将 BirthChart/CosmicWeather/Tools/Synastry/Wiki/FeaturedArticles/Ask/SocialProof/Newsletter/Footer 改为 viewport-deferred lazy loading，首页首屏不再立即下载首屏外重 chunk；HeroTodayCard 改为 5s+idle 后请求其 chunk 与 `/api/astro/today`；Hero LCP 标题改用系统衬线；FooterSection 改用 `/brand/logo-mark-64.png` 小图。
