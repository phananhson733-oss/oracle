<!-- INPUT: scripts/ 下构建脚本（generate-seo-pages.mjs）与 vite.config.ts 复用的纯函数。 -->
...
| `faq-jsonld.mjs` | `buildFaqPageSchema(faqs)` / `parseFaqsFromMarkdown(md)` / `buildFaqSchemaFromMarkdown(md)` —— 把文章正文 FAQ 段解析成 question/answer 对（正则镜像 `WikiArticleDetailPage` 的 SPA 内联解析，保证 stub/SPA 同发一份 FAQPage），按 ≥2 门槛构造 schema.org FAQPage 对象，交给 `safe-jsonld.mjs` 序列化后注入静态 stub 拿 FAQ 富结果。已由 `generate-seo-pages.mjs` 的 writeArticle 调用（T6）。回归测试见 `tests/unit/faq-jsonld.test.ts`。 |
| `adsense-head-tag.mjs` | `buildAdsenseHeadTag(env)` —— AdSense `<head>` loader 的唯一真相源。两级门控（`VITE_ADSENSE_HEAD_LOADER_ENABLED === 'true'` + `VITE_ADSENSE_CLIENT_ID` 匹配 `^ca-pub-\d{10,25}$`）任一不过即返回空串。被 `generate-seo-pages.mjs`（442 静态 stub）与 `vite.config.ts`（SPA 壳）共用，避免门控逻辑漂移。返回值直接进原始 HTML，格式校验是唯一注入防线 —— 非法输入整体拒绝而非转义放行。回归测试见 `tests/unit/adsense-head-tag.test.ts`。 |
