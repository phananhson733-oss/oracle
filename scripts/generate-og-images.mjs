...
// OUTPUT: 为每篇文章按语言生成 1200×630 品牌 OG 图，写入 public/og/articles/<slug>.png|.webp（zh 用 <slug>.zh.png|.webp），并返回写入清单；幂等（标题+作者+分类+品牌调色板 hash 未变则跳过）。
...
// Editorial warm-paper system: paper ground + ink headline + aged-gold accent.
const COLORS = {
  bg: '#F4EFE4', // --space-950 paper ground (rgb 244 239 228)
  panel: '#FBF8F1', // raised paper surface for the glow gradient
  title: '#16130F', // ink — headline on paper
  accent: '#9A7B3F', // accent.DEFAULT aged gold（陈金）
  muted: '#6B6053', // ink-60 muted text
  rule: 'rgba(154, 123, 63, 0.4)', // accent rule
};
...
// palette 并入 hash：改品牌调色板时旧缓存自动失效重渲（否则暗色旧图会永久驻留）。
// palette 放在 spread 之后：任何未来的 params.palette 键都不可能静默遮蔽品牌调色板。
export const contentHash = (params) =>
  crypto.createHash('sha256').update(JSON.stringify({ ...params, palette: COLORS })).digest('hex').slice(0, 16);
