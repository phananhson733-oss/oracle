<!-- INPUT: 项目根目录文件与子目录结构（含助手语言规则、积分入口、报告积分记录与迁移提示更新）。 -->
<!-- OUTPUT: 根目录架构摘要与文件索引（含助手语言规则与积分入口、报告积分记录）。 -->
...
- AGENTS.md｜地位：助手入口文档｜功能：指向 OpenSpec 助手说明与语言规则。
- App.tsx｜地位：主应用入口组件｜功能：组合路由与页面（含积分入口/使用情况页、迁移提示与设置页 GM 测试/开发会话指令）。
