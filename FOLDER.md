<!-- INPUT: 项目根目录文件与子目录结构（含 Tailwind/PostCSS 构建文件、短链跳转与 UI/语言规则更新）。 -->
<!-- OUTPUT: 根目录架构摘要与文件索引（含样式构建配置、短链跳转、支付成功路由放行、PayPal 回跳处理与规范门槛）。 -->
...
- App.tsx｜地位：主应用入口组件｜功能：组合路由与页面（含 `/go/:code` 短链跳转、积分入口/使用情况页、地名输入建议、迁移提示与设置页 GM 测试/开发会话指令）。
...
近期更新
- 新增 `/go/:code` 短链跳转入口，支持 link-attribution 工具生成的同站 `to` 回退链接，并预留 code registry。
