<!-- INPUT: 项目根目录文件与子目录结构（含 SEO/GEO 静态页、www 重定向、SEO head 回切与 UI 规范入口）。 -->
<!-- OUTPUT: 根目录架构摘要与文件索引（含 SEO/GEO 静态页、SEO head 调整与 UI 规范门槛）。 -->
...
- App.tsx｜地位：主应用入口组件｜功能：组合路由与页面（含积分入口/使用情况页、地名输入建议、迁移提示与设置页 GM 测试/开发会话指令）。
