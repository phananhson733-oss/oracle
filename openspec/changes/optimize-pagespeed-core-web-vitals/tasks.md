...
- [x] 1.2 Record the current deployed asset list from `/` and verify it matches the worktree build output, or document the deploy/worktree mismatch before implementation.
  - Deploy/worktree mismatch documented (2026-07-07): live `/` serves `/assets/index-DPOlUXZR.js`, `/assets/react-vendor-C2iShpWN.js`, `/assets/index-BX-EnIGd.css`, still contains `body.loading-fonts`, old broad `display=swap` Google Fonts URL, and schema `logo: https://www.astrologywiki.com/logo.png`. Current worktree build serves `/assets/index-DykwA-tK.js`, `/assets/react-vendor-CONxsY8T.js`, `/assets/index-CMawwnko.css`, uses `display=optional`, and schema logo `/brand/logo-schema-512.png`.
...
- [ ] 3.4 Recheck LCP breakdown: the `"modern psychology."` hero text should no longer have a large element render delay.
  - Local browser check (2026-07-07): font request reduced to Readex 400/500/600, Cormorant 400/500, IBM Plex Mono 400/500 with `display=optional`; local first-viewport CLS dropped to ~0.0019. Deploy Lighthouse/PageSpeed LCP breakdown remains pending.
...
- [ ] 4.5 Recheck PageSpeed unused JS, main-thread work, and network dependency tree after changes.
  - Local production build + Playwright network check (2026-07-07): moved sign calculators, auth/payment modals, and payment success pages out of the initial app chunk; viewport-deferred all landing below-the-fold sections so BirthChart/city-search/CosmicWeather/Tools/Synastry/Wiki/FeaturedArticles/Ask/SocialProof/Newsletter/Footer chunks and the 3.36MB wiki/article chunk are no longer requested during the first viewport. Deploy PageSpeed recheck remains pending.
...
- [x] 5.2 Reduce unused CSS for the home route where feasible without destabilizing Tailwind scanning.
  - Replaced Wiki dynamic `color_token` Tailwind gradient classes with a controlled inline-gradient helper and removed `backend/src/data/wiki.ts` from Tailwind content scanning. Local production build CSS dropped from 152,661 raw / 21,911 gzip / 16,695 brotli bytes to 142,012 raw / 20,885 gzip / 15,957 brotli bytes.
...
- [x] 7.1 Run the relevant build command and record output. If using the full `npm run build` would ping IndexNow, use the closest non-pinging build path and explain the difference.
