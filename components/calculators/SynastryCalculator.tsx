...
//        PersonBirthFields（共享双人表单）、synastryData（纯数据组装）、共享原语 ToolPageShell / SynastryResultView。
// OUTPUT: 合盘（Synastry）计算器——两人出生表单 → 两次匿名 natal → 客户端交叉相位/宫位覆盖 → 数据型结果页。
...
export const SynastryCalculator: React.FC = () => {
  ...
  const [result, setResult] = useState<SynastryResultData | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nextResult = buildSynastryResultData({
        labelA: nextLabelA,
        labelB: nextLabelB,
        birthA,
        birthB,
      ...
      trackEvent("synastry_calculated", {
        ...
        overlay_count: nextResult.overlays.length,
      });
      setTimeout(() => {
