<!-- INPUT: components/UIComponents（useLanguage/useTheme）、services/analytics（trackEvent）、react-router-dom（Link）、toolsCatalog 工具目录数据。 -->
...
定位
- 计算器矩阵的统一发现入口（hub-and-spoke 内链中枢）。匿名可访问、公开可索引，呈现为正式工具图谱：顶部 atlas hero + 可信说明 + 5 个主题分类，每类一句话引导 + 编号工具卡片网格。
...
- ToolsHubPage.tsx｜地位：hub 页组件（路由 /:lang/tools）｜功能：顶部 atlas-style hero（中英文文案 + 星盘轮 SVG + 可信说明条）+ featured「Guided Reading / 引导式解读」(Synthetica) 入口卡 + 5 分类 section（每类引导文案 + `n° 01` 编号工具卡片网格），卡片语言前缀内链到各计算器（`/${language}/${slug}`），含 16 个单色描边图标；trackEvent 仅送 slug+location（无 PII）。featured Synthetica 是带每日配额的 AI 解读、链接 `/wiki?tab=tools`，刻意不进 `toolsCatalog`（catalog 与可爬取计算器路由 1:1），守恒测试见 `tests/unit/tools-hub-synthetica.test.tsx`；/tools 是 Synthetica 的唯一入口（wiki tools tab 已合并移除）。
