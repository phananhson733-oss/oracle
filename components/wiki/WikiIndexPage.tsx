...
const WikiIndexPage: React.FC = () => {
  ...
  const formatText = (value: string) =>
    language === "zh" ? stripLatin(value) : value;
  const getPanelDescription = (value: string) =>
    pickLastSentence(formatText(value));
  const getDisplaySymbol = (item: WikiItemSummary) => {
    const rawSymbol = ["planets", "signs", "points", "asteroids"].includes(
      item.type,
    )
  ...
  const groupByType = (type: WikiItemType) =>
  ...
  const renderFeatureCard = (item: WikiItemSummary) => {
    const detail = item.subtitle
      ...
      : getPanelDescription(item.description);
    ...
    return renderCardShell(
      ...
      <div className="p-5 flex items-start gap-4">
        <div
          className={`w-12 h-12 rounded-2xl border ${borderColor} flex items-center justify-center text-2xl ${theme === "dark" ? "bg-space-900/70" : "bg-paper-100/85"}`}
        >
          {displaySymbol}
        </div>
        <div className="flex-1 min-w-0 space-y-2">
          <div className="text-[10px] uppercase tracking-[0.3em] text-gold-500">
            {t.wiki.type_labels[item.type]}
          </div>
          <div className="text-lg font-serif font-semibold">{displayTitle}</div>
          {detail && (
            <div className={`text-xs ${mutedText} line-clamp-2`}>{detail}</div>
          )}
        </div>
        <ArrowUpRight
          className={`opacity-0 group-hover:opacity-100 transition-opacity ${mutedText}`}
          size={16}
        />
      </div>,
      "min-h-[120px]",
    ...
    return renderCardShell(
      ...
      <div className={`h-full flex flex-col justify-between ${config.padding}`}>
        <div className="flex items-start justify-between">
          <div className={`${config.symbol} drop-shadow-lg ${symbolTone}`}>
            {displaySymbol}
          </div>
          <ArrowUpRight
            className={`opacity-0 group-hover:opacity-100 transition-opacity ${mutedText}`}
        ...
        <div className="space-y-1">
          <div className={`font-serif font-semibold ${config.title}`}>
            {displayTitle}
          </div>
          {showSubtitle && displaySubtitle && (
            <div className={`${config.subtitle} ${mutedText} line-clamp-2`}>
              {displaySubtitle}
            </div>
          )}
        </div>
    ...
    return renderCardShell(
      ...
      <div className="p-4 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          {displayDescription && (
            <div className={`text-xs ${mutedText} line-clamp-2`}>
              {displayDescription}
            </div>
          )}
        </div>
  ...
  const renderTopicCard = (item: WikiItemSummary) => {
    ...
    return renderCardShell(
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="text-lg font-serif font-semibold">{displayTitle}</div>
        {displayDescription && (
          <div className={`text-xs leading-relaxed ${mutedText} line-clamp-3`}>
            {displayDescription}
          </div>
        )}
      </div>,
  ...
  return (
    <div className="space-y-16">
      ...
      <section className="text-center space-y-6 pt-6">
        <div
          className={`text-xs uppercase tracking-[0.35em] ${theme === "dark" ? "text-gold-400" : "text-gold-600"}`}
        >
          {t.wiki.kicker}
        </div>
        <h2 className="text-4xl md:text-6xl font-serif font-semibold">
          {t.wiki.library_title}
        </h2>
        <p className={`text-sm md:text-base ${mutedText} max-w-2xl mx-auto`}>
          {t.wiki.library_subtitle}
        </p>
      ...
      {!loading && !error && searchTerm.trim() && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          {filteredItems.length === 0 && (
            <Card className={`col-span-full text-center text-sm ${mutedText}`}>
              {t.wiki.library_empty}
            </Card>
          )}
        </div>
      )}
      ...
      {!loading && !error && !searchTerm.trim() && (
        <div className="space-y-16">
          ...
          <Section title={t.wiki.section_planets}>
            <div
              id="planets"
              className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 scroll-mt-24"
            >
          ...
          <Section title={t.wiki.section_points}>
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
              {[...points, ...asteroids].map((item) =>
                renderTileCard(item, "sm"),
              )}
            </div>
          </Section>
          ...
          <Section title={t.wiki.section_signs}>
            <div
              id="signs"
              className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 scroll-mt-24"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <Section title={t.wiki.section_houses} className="lg:col-span-2">
              <div
                id="houses"
                className="grid grid-cols-3 sm:grid-cols-4 gap-3 scroll-mt-24"
              >
