...
const shouldSkipLine = (line: string): boolean => {
  if (!line) return false;
  if (line.includes('请继续') || line.includes('生成后续') || line.includes('生成最后')) return true;
  if (/^\*?\(?\s*(Part|第)\s*\d+.*(结束|请继续|生成).*\)?\*?$/i.test(line)) return true;
  return false;
};
...
const parseMarkdownBlocks = (content: string): MarkdownBlock[] => {
  ...
  const flushParagraph = () => {
    ...
    const text = stripReportMarkers(removeEmoji(paragraph.join(' ').replace(/\s+/g, ' ').trim()).trim());
  ...
  const flushPre = () => {
    ...
    blocks.push({ type: 'pre', lines: preLines.map(line => removeEmoji(line)) });
    preLines = [];
  };
  ...
  for (const rawLine of lines) {
    ...
    if (isDividerLine(line) || isDividerLine(compactLine)) {
      ...
      flushList();
      flushPre();
      blocks.push({ type: 'divider', weight: compactLine.length > 6 ? 'heavy' : 'light' });
      continue;
    }
    ...
    if (orderedMatch) {
      ...
      if (itemText) {
        ...
        if (!list || !list.ordered) {
          flushList();
          list = { ordered: true, items: [], start: parseInt(orderedMatch[1], 10) || 1 };
        }
...
const extractHeadingParts = (text: string): { label?: string; title: string } => {
  const trimmed = text.trim();
  const cleaned = trimmed.replace(/^\*+/, '').replace(/\*+$/, '').trim();
  const numericMatch = cleaned.match(/^(\d+(?:\.\d+)*)(?:[.)、．]\s*|\s+)(.+)$/);
  if (numericMatch) {
    return { label: numericMatch[1], title: numericMatch[2].trim() };
  }
  const chineseMatch = cleaned.match(/^([一二三四五六七八九十]+)[、.．]\s*(.+)$/);
  if (chineseMatch) {
...
export const WikiClassicDetailPage: React.FC = () => {
  ...
  const getHeadingSpacing = (level: number, prevBlock?: MarkdownBlock): React.CSSProperties => {
    const prevHeadingLevel = prevBlock?.type === 'heading' ? prevBlock.level : null;
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return (
          <strong
            key={i}
            className={`font-semibold ${palette.heading}`}
          >
            {part.slice(2, -2)}
          </strong>
        );
  ...
  const renderBlock = (block: MarkdownBlock, index: number, isLead: boolean, prevBlock?: MarkdownBlock) => {
    ...
    if (block.type === 'list') {
      ...
      if (block.ordered) {
        const orderedStyle: React.CSSProperties = {
          ...
          counterReset: block.start && block.start > 1 ? `list-item ${block.start - 1}` : undefined
        };
        return (
          <ol
            key={index}
      ...
      return (
        <ul
          ...
          style={{ ...listSpacing, listStyleType: 'disc', listStylePosition: 'outside' }}
        >
          {block.items.map((item, i) => (
            <li key={i} style={{ display: 'list-item' }}>
              {renderInlineText(item)}
  ...
  const renderPage = (blocks: MarkdownBlock[]) => {
    return (
      <div
        ...
        style={{ ...pageSize, backgroundImage: paperBackground, boxShadow: pageShadow }}
      >
        <div
          className="absolute inset-0 opacity-35 pointer-events-none"
          style={{ backgroundImage: paperNoise, backgroundSize: '120px 120px' }}
        />
        <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: paperEdge }} />
        <div className="relative z-10 flex h-full flex-col font-serif" style={pagePadding}>
          <div className={`flex items-center text-[0.62rem] uppercase tracking-[0.38em] font-sans ${palette.inkMuted}`}>
