...
export const RelatedArticles: React.FC<RelatedArticlesProps> = ({
  ...
  const getRelationLabel = (relation: RelationType): string => {
    const labels: Record<RelationType, string> = {
      ...
      "natural-correspondence":
        t.wiki?.relation_correspondence || "Correspondence",
    };
  ...
  const getItemIcon = (id: string, type: WikiItemType): string => {
    return forceTextSymbol(
      UNICODE_SYMBOLS[id] || TYPE_FALLBACK_ICONS[type] || "✦",
    );
  };
  ...
  return (
    <section className="related-articles">
      <h3
        className={`text-lg font-semibold mb-4 ${isDark ? "text-star-100" : "text-paper-800"}`}
      >
        ...
        {relatedItems.map((item) => (
          <Link
            ...
            onClick={() =>
              trackEvent("wiki_related_article_clicked", {
                article_id: item.id,
                article_title: getItemDisplayName(item.id, item.type),
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div
                className={`font-medium truncate ${isDark ? "text-star-100" : "text-paper-800"}`}
              >
