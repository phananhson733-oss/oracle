...
const buildShelfStyle = (theme: 'dark' | 'light') => {
  ...
  return {
    ...
    backgroundColor: isDark ? 'rgba(15, 18, 28, 0.95)' : 'rgba(253, 251, 247, 0.98)',
...
const WikiClassicsPage: React.FC = () => {
  ...
  const renderCover = (item: WikiClassicSummary, variant: 'featured' | 'shelf' | 'compact' = 'shelf') => {
    if (item.cover_url) {
      return (
        <img
          src={item.cover_url}
    ...
    // 作者字体大小
    const authorSize = isCompact ? 'text-[10px]' : isFeatured ? 'text-xs' : 'text-[11px]';
          ...
          <svg width="100%" height="100%" preserveAspectRatio="none">
            <defs>
              <linearGradient id={`grad-${item.id}`} x1="0" y1="0" x2="1" y2="1">
                ...
                <stop offset="100%" stopColor={isDark ? '#0F121C' : '#EBE9E4'} />
              </linearGradient>
              <pattern id={`stars-${item.id}`} x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
                <circle cx="10" cy="10" r="0.5" fill={svgFrame} fillOpacity="0.3" />
                <circle cx="30" cy="25" r="0.3" fill={svgFrame} fillOpacity="0.2" />
                <circle cx="20" cy="35" r="0.4" fill={svgFrame} fillOpacity="0.25" />
              </pattern>
              <pattern id={`dots-${item.id}`} x="0" y="0" width="6" height="6" patternUnits="userSpaceOnUse">
                <circle cx="3" cy="3" r="0.3" fill={svgFrame} fillOpacity="0.15" />
              </pattern>
            </defs>
            ...
            {/* 书脊阴影 */}
            <rect x="0" y="0" width="10%" height="100%" fill="black" fillOpacity={isDark ? 0.25 : 0.1} />
            <line x1="10%" y1="0" x2="10%" y2="100%" stroke={svgFrame} strokeOpacity="0.3" strokeWidth="1" />

            {/* 装饰边框 */}
            <rect x="15%" y="5%" width="80%" height="90%" fill="none" stroke={svgFrame} strokeWidth="1" strokeOpacity="0.25" rx="4" />

            {/* 占星符号装饰 - 移到下方 */}
            <circle cx="55%" cy="75%" r={isFeatured ? '12%' : '10%'} fill="none" stroke={svgFrame} strokeWidth="0.5" strokeOpacity="0.12" />
            <circle cx="55%" cy="75%" r={isFeatured ? '8%' : '6%'} fill="none" stroke={svgFrame} strokeWidth="0.5" strokeOpacity="0.08" />
          </svg>
        </div>

        {/* 内容区域 - 在内框范围内 */}
        <div className="absolute inset-0 z-10" style={{ left: '15%', top: '5%', right: '5%', bottom: '5%' }}>
          <div className="relative h-full w-full flex flex-col">
            {/* 标题 - 顶部居中 */}
            <div className={`flex-shrink-0 pt-3 px-2 text-center`}>
              <div className={`${titleSize} font-serif font-semibold leading-tight line-clamp-4 ${isDark ? 'text-gold-50' : 'text-paper-900'}`}>
                {item.title}
              </div>
            </div>
            ...
            <div className={`flex-shrink-0 pb-6 px-2 text-center`}>
              <div className={`${authorSize} ${mutedText} uppercase tracking-wider font-medium`}>
                {item.author}
              </div>
            </div>
          </div>
      ...
      >
        ...
        >
          ...
          <div className="absolute right-0 top-1 bottom-1 w-1 z-10">
            <div className={`h-full w-full ${isDark ? 'bg-gradient-to-l from-white/10 to-transparent' : 'bg-gradient-to-l from-black/5 to-transparent'}`} />
          </div>
        ...
        {/* 作者标签（悬停显示） - 优化可读性 */}
        <div className={`mt-3 text-xs uppercase tracking-[0.15em] ${mutedText} opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-1 group-hover:translate-y-0 font-medium`}>
          {item.author}
        </div>
      </Link>
    );
    ...
    <div className="space-y-12 max-w-6xl mx-auto">
      ...
      <header className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] items-center">
        ...
        <div className="space-y-6">
          ...
          {/* 标题 */}
          <h2 className={`text-4xl md:text-5xl lg:text-6xl font-serif font-semibold leading-[1.1] tracking-tight ${headingText}`}>
            {t.wiki.classics_title}
          </h2>

          {/* 描述 */}
          <div className="flex items-start gap-4">
            <div className={`w-0.5 h-full min-h-[60px] rounded-full ${isDark ? 'bg-gradient-to-b from-gold-500/60 to-gold-500/10' : 'bg-gradient-to-b from-gold-600/60 to-gold-600/10'}`} />
            <p className={`text-sm md:text-base ${mutedText} max-w-lg leading-relaxed`}>
        ...
        <div className="relative">
          ...
          {/* 展示卡片 */}
          <div className={`relative rounded-[2rem] border ${frameBorder} ${panelSurface} backdrop-blur-md shadow-2xl overflow-hidden`}>
            {/* 顶部装饰条 */}
            <div className={`h-1 w-full ${isDark ? 'bg-gradient-to-r from-transparent via-gold-500/40 to-transparent' : 'bg-gradient-to-r from-transparent via-gold-600/40 to-transparent'}`} />

            <div className="p-6">
              {/* 区块标题 */}
              <div className="flex items-center justify-between mb-2">
                <div className={`text-[11px] uppercase tracking-[0.25em] ${mutedText} flex items-center gap-2`}>
      ...
      >
        <Card className={`relative overflow-hidden shadow-2xl rounded-[2rem] border ${frameBorder}`} noPadding>
          {/* 书架背景 */}
          <div className={`absolute inset-0 ${isDark ? 'bg-space-950' : 'bg-paper-50'}`} />
          <div className="absolute inset-0" style={shelfStyle} />

          {/* 顶部灯光效果 */}
          <div className={`absolute top-0 inset-x-0 h-32 bg-gradient-to-b ${isDark ? 'from-gold-500/8' : 'from-gold-500/10'} to-transparent pointer-events-none`} />

          {/* 书架木边 */}
          <div className={`absolute top-0 inset-x-4 h-2 rounded-b-lg ${isDark ? 'bg-gradient-to-b from-amber-900/30 to-amber-950/20' : 'bg-gradient-to-b from-amber-800/20 to-amber-900/10'} shadow-[0_2px_8px_rgba(0,0,0,0.2)]`} />
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
                {Array.from({ length: 10 }).map((_, index) => (
                  <div key={index} className="space-y-3 animate-pulse">
                    <div className={`aspect-[3/4] rounded-lg ${isDark ? 'bg-space-800/60' : 'bg-paper-200/60'}`} />
                  </div>
                ))}
              </div>
            )}
                  ...
                  <div key={group.category} className="space-y-5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className={`text-xs uppercase tracking-[0.3em] font-medium ${highlightText}`}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-6 md:gap-8">
                      {group.items.map((item, index) => renderBookCard(item, index + groupIndex))}
                    </div>
          ...
          {/* 底部书架木边 */}
          <div className={`absolute bottom-0 inset-x-4 h-3 rounded-t-lg ${isDark ? 'bg-gradient-to-t from-amber-900/40 to-amber-950/20' : 'bg-gradient-to-t from-amber-800/25 to-amber-900/10'} shadow-[0_-2px_8px_rgba(0,0,0,0.15)]`} />
        </Card>
      </Section>
    </div>
  );
