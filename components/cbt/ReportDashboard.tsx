...
const normalizeComparisonText = (value: string) => stripTrailingPunct(value)
  .replace(/[：:]/g, '')
  .replace(/[，,、]/g, '')
  .replace(/\s+/g, '')
  .toLowerCase();
...
const buildAstroAspectLines = (text: string, language: Language) => {
  ...
  segments.forEach(segment => {
    ...
    if (key === 'other') {
      ...
      const prefix = otherLabel ? `${otherLabel}${language === 'zh' ? '：' : ': '}` : '';
...
const splitInterpretationSections = (text: string) => {
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      // 普通文本行，跳过与标签关键词相同的内容
      const isLabelKeyword = new RegExp(`^\\s*(${INTERPRETATION_SECTION_PATTERN})\\s*[:：]*\\s*$`, 'i').test(line);
      if (!isLabelKeyword) {
...
const ReportDashboard: React.FC<ReportDashboardProps> = ({ record, report, onUpdate }) => {
  ...
  const chartAxis = isLight ? 'rgb(var(--star-400) / 0.9)' : 'rgb(var(--star-200) / 0.9)';
  const chartTooltip = {
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in">
      ...
      {/* 情绪波动卡片 */}
      <div className={`rounded-xl border border-l border-l-gold-500/40 p-6 ${isLight ? 'bg-paper-100/85 border-paper-300' : 'bg-space-900/60 border-gold-500/20'}`}>
        ...
        <div className="h-28">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Tooltip contentStyle={chartTooltip} itemStyle={{ color: 'rgb(var(--star-50) / 1)' }} />
              <Line
                type="monotone"
      ...
      {/* 1. 认知评估卡片 */}
      <div className={`rounded-xl border border-l border-l-accent/40 p-6 ${isLight ? 'bg-paper-100/85 border-paper-300' : 'bg-space-900/60 border-gold-500/20'}`}>
        <div className={`flex items-center gap-3 mb-4 pb-3 border-b ${isLight ? 'border-paper-200' : 'border-gold-500/15'}`}>
          <div className={`w-9 h-9 rounded-xl border flex items-center justify-center ${isLight ? 'border-accent/30 bg-paper-100/85 text-accent' : 'border-accent/30 bg-space-950 text-accent'}`}>
            <Brain size={18} />
          </div>
      ...
      {/* 2. 平衡性见地卡片 */}
      <div className={`rounded-xl border border-l border-l-success/40 p-6 ${isLight ? 'bg-paper-100/85 border-paper-300' : 'bg-space-900/60 border-gold-500/20'}`}>
        <div className={`flex items-center gap-3 mb-4 pb-3 border-b ${isLight ? 'border-paper-200' : 'border-gold-500/15'}`}>
          <div className={`w-9 h-9 rounded-xl border flex items-center justify-center ${isLight ? 'border-success/30 bg-paper-100/85 text-success' : 'border-success/30 bg-space-950 text-success'}`}>
            <Target size={18} />
          </div>
        ...
        <div className="pl-12 space-y-4">
          {record.balancedEntries.map(entry => (
            <div key={entry.id} className={`pb-4 border-b last:border-0 last:pb-0 ${isLight ? 'border-paper-200' : 'border-gold-500/15'}`}>
      ...
      <div className={`rounded-xl border border-l border-l-gold-500/40 p-6 ${isLight ? 'bg-paper-100/85 border-paper-300' : 'bg-space-900/60 border-gold-500/20'}`}>
        ...
        <div className="pl-12 space-y-3">
          {aspectLines.length > 0 ? aspectLines.map((line, i) => (
            <div key={i} className="flex gap-3 items-start">
              <div className={`shrink-0 w-1.5 h-1.5 rounded-full mt-2 ${isLight ? 'bg-gold-600/50' : 'bg-gold-500/50'}`} />
              <span className={`text-sm leading-relaxed ${isLight ? 'text-paper-700' : 'text-star-200'}`}>
                {line.label && <span className={`font-medium ${isLight ? 'text-gold-700' : 'text-gold-400'}`}>{line.label}: </span>}
                {line.text}
              </span>
            </div>
          )) : (
          ...
          {(interpretationList.intro || interpretationList.items.length > 0) && (
            <div className={`mt-4 pt-4 border-t space-y-2 ${isLight ? 'border-paper-200' : 'border-gold-500/15'}`}>
      ...
      {/* 4. 执行建议卡片 */}
      <div className={`rounded-xl border border-l border-l-star-200/40 p-6 ${isLight ? 'bg-paper-100/85 border-paper-300' : 'bg-space-900/60 border-gold-500/20'}`}>
        <div className={`flex items-center gap-3 mb-4 pb-3 border-b ${isLight ? 'border-paper-200' : 'border-gold-500/15'}`}>
          <div className={`w-9 h-9 rounded-xl border flex items-center justify-center ${isLight ? 'border-star-200/30 bg-paper-100/85 text-star-200' : 'border-star-200/30 bg-space-950 text-star-200'}`}>
