<!-- INPUT: CBT 组件目录结构与职责说明（含统计解读权限校验、统计入口卡片精简展示、AI 文本清理、月份导航统一、无记录占位、情绪配色优化、行运分组补齐与报告卡片色条收窄）。 -->
<!-- OUTPUT: CBT 组件目录文档（含统计解读权限校验与统计入口精简展示、AI 文本净化更新说明、占星解读分组与标签去重、行运分行高亮与暗色边框对齐）。 -->
