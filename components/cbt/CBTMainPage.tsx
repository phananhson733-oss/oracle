...
import { FrameworkDisclaimer } from "../shared/FrameworkDisclaimer";
import {
  SomaticPatternView,
  SourceSupportView,
  MoodCompositionView,
...
const MOOD_IMAGES: MoodImages = {
  very_happy:
    "https://images.unsplash.com/photo-1534447677768-be436bb09401?w=400&h=400&fit=crop&auto=format&q=75",
  happy:
    "https://images.unsplash.com/photo-1464802686167-b939a67e06a1?w=400&h=400&fit=crop&auto=format&q=75",
  okay: "https://images.unsplash.com/photo-1506318137071-a8e063b4bc02?w=400&h=400&fit=crop&auto=format&q=75",
  annoyed:
    "https://images.unsplash.com/photo-1502134249126-9f3755a50d78?w=400&h=400&fit=crop&auto=format&q=75",
  terrible:
    "https://images.unsplash.com/photo-1462331940025-496dfbfc7564?w=400&h=400&fit=crop&auto=format&q=75",
};
...
const CBTMainPage: React.FC<CBTMainPageProps> = ({ profile }) => {
  ...
  const shellTone = isLight
    ? "bg-paper-100/90 border-gold-600/30"
    : "bg-space-900/40 border-gold-500/10";
  const sidebarTone = isLight
    ? "bg-paper-100/80 border-gold-600/30"
    : "bg-space-900/20 border-gold-500/10";
  ...
  const { checkAccess: checkStatsAccess } = useFeatureAccess(
    "cbt_stats",
    statsFeatureId,
  );
            ...
            try {
              ...
              if (Array.isArray(parsed) && parsed.length > 0) {
                ...
                const realRecords = parsed.filter(
                  (r: CBTRecord) => !r.id?.startsWith("mock-"),
                );
  ...
  const handleOpenAnalysis = React.useCallback(
    async (view: "card1" | "card2" | "card3" | "card4") => {
      // LOGIN_GATE_MODE: 未登录时弹出登录提醒
      if (LOGIN_GATE_MODE && !isAuthenticated) {
        openLoginModal(
          t.login_gate?.unlock_cbt_stats ||
  ...
  const startNewEntry = (date?: Date) => {
    ...
    trackEvent("cbt_module_started", {
      entry_date: targetDateValue.toISOString().slice(0, 10),
    });
  };
  ...
  if (isLoading) {
    return (
      <OracleLoading variant="fullscreen" thinkingLabel={t.journal.loading} />
    );
  }
  ...
  if (history.length === 0) {
    return (
      <div className="min-h-screen w-full bg-space-950 text-star-50 flex flex-col items-center px-4 md:px-10 pt-12 md:pt-12 pb-6 font-sans">
        <div className="w-full max-w-7xl">
          <FrameworkDisclaimer />
        </div>
        <div
          className={`w-full max-w-7xl h-[90vh] backdrop-blur-3xl rounded-[3.5rem] border shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden relative ${shellTone}`}
        >
  ...
  return (
    <div className="min-h-screen w-full bg-space-950 text-star-50 flex flex-col items-center px-4 md:px-10 pt-12 md:pt-12 pb-6 font-sans">
      ...
      <div
        className={`w-full max-w-7xl h-[90vh] backdrop-blur-3xl rounded-[3.5rem] border shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col md:flex-row relative ${shellTone}`}
      >
        <aside
          className={`w-full md:w-[400px] border-r flex flex-col relative z-0 ${sidebarTone}`}
        >
          <div
            className={`p-8 border-b ${isLight ? "border-gold-600/30" : "border-gold-500/10"} flex items-center justify-between`}
          >
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-3xl bg-gradient-to-tr from-gold-600 to-gold-400 shadow-xl flex items-center justify-center p-0.5">
                <div className="w-full h-full rounded-[1.2rem] bg-space-900 flex items-center justify-center font-serif font-black text-xl text-star-50">
                  {initials}
                </div>
              </div>
              <div>
                <h2 className="text-lg font-black text-star-50 tracking-tight">
                  {displayName}
                </h2>
                {profile?.zodiac && (
                  <div
                    className={`text-xs uppercase tracking-[0.2em] font-bold mt-0.5 ${accentText}`}
                  >
                    {profile.zodiac}
                  </div>
                )}
