...
const isNatalOverviewContent = (value: unknown): value is NatalOverviewContent => (
  isRecord(value)
...
const LEGACY_ZH_SIGNS = ['白羊', '金牛', '双子', '巨蟹', '狮子', '处女', '天秤', '天蝎', '射手', '摩羯', '水瓶', '双鱼'];
const LEGACY_EN_SIGNS = ['Aries', 'Taurus', 'Gemini', 'Cancer', 'Leo', 'Virgo', 'Libra', 'Scorpio', 'Sagittarius', 'Capricorn', 'Aquarius', 'Pisces'];
...
const DEFAULT_DESC: Record<'sun' | 'moon' | 'rising', { zh: string; en: string }> = {
  sun: { zh: '这是你本命盘的核心动力来源。', en: 'This is a core source of your natal drive.' },
  moon: { zh: '这里反映你的情绪节奏与安全需求。', en: 'This reflects your emotional rhythm and needs.' },
  rising: { zh: '这里展现你被他人感知的方式。', en: 'This shapes how others experience you.' },
};

const extractLegacySign = (description: string, language: Language) => {
  if (!description) return undefined;
  if (language === 'zh') {
    const match = description.match(new RegExp(`(${LEGACY_ZH_SIGNS.join('|')})座`));
    return match?.[1];
  }
  const match = description.match(new RegExp(`\\b(${LEGACY_EN_SIGNS.join('|')})\\b`, 'i'));
...
const convertLegacyNatalOverview = (legacy: Record<string, unknown>, language: Language): NatalOverviewContent => {
  ...
  const interpretation = isRecord(legacy.interpretation) ? legacy.interpretation : {};
  const overviewText = isString(interpretation.overview) ? interpretation.overview : undefined;

  const buildBig3 = (key: 'sun' | 'moon' | 'rising') => {
    const entry = (big3 as Record<string, unknown>)[key];
    const description = resolveLegacyDescription(entry) || DEFAULT_DESC[key][language];
    const sign = extractLegacySign(description, language);
    const label = BIG3_LABELS[key][language];
    const title = sign
      ? (language === 'zh' ? `${label}${sign}` : `${label} in ${sign}`)
      : label;
    return {
      title,
  ...
  const mercuryText = resolveLegacyDescription((personalPlanets as Record<string, unknown>).mercury);
  const venusText = resolveLegacyDescription((personalPlanets as Record<string, unknown>).venus);

  return {
    sun: buildBig3('sun'),
    moon: buildBig3('moon'),
    rising: buildBig3('rising'),
    core_melody: language === 'zh'
      ? { keywords: ['动力', '感受'], explanations: ['行动与情绪并重', '保持内在觉察'] }
      : { keywords: ['drive', 'sensitivity'], explanations: ['Balances action with feeling', 'Stays internally aware'] },
    top_talent: {
      title: language === 'zh' ? '思维与表达' : 'Mind & Expression',
      example: mercuryText || overviewText || (language === 'zh' ? '你的优势往往体现在表达与洞察上。' : 'Your strengths show up in how you express and interpret.'),
      advice: language === 'zh' ? '把优势转化为清晰可执行的目标。' : 'Channel it into clear, practical goals.',
    },
    top_pitfall: {
      title: language === 'zh' ? '关系张力' : 'Relational Tension',
      triggers: venusText
        ? [language === 'zh' ? '投入过深' : 'Over-investing', language === 'zh' ? '期待落差' : 'Expectation gaps']
        : [language === 'zh' ? '节奏失衡' : 'Overload', language === 'zh' ? '情绪起伏' : 'Emotional swings'],
      protection: language === 'zh' ? '放慢节奏，先稳定情绪再行动。' : 'Slow down, steady emotions before acting.',
    },
    trigger_card: {
      auto_reactions: language === 'zh' ? ['先防御', '先解释'] : ['defend quickly', 'over-explain'],
      inner_need: language === 'zh' ? '被理解与被接住' : 'to be understood and supported',
      buffer_action: language === 'zh' ? '先停三秒再回应。' : 'Pause for three seconds before responding.',
    },
    share_text: overviewText || (language === 'zh' ? '我的星盘提醒我在行动与感受间寻找平衡。' : 'My chart reminds me to balance action with sensitivity.'),
  };
};

const normalizeNatalOverviewContent = (value: unknown, language: Language): NatalOverviewContent | null => {
